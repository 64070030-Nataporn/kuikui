...
var app = new Vue({
  ...
  data: {
    ...
    typemovie: [
      {
        "typename": "Sci-Fi",
        "imgtype": "/resources/image 10.png"
      },
      {
        "typename": "Comedy",
        "imgtype": "/resources/image 14.png"
      },
      {
        "typename": "Horror",
        "imgtype": "/resources/image 15.png"
      },
      {
        "typename": "Action",
        "imgtype": "/resources/image 16.png"
      },
      {
        "typename": "Drama",
        "imgtype": "/resources/image 17.png"
      }
    ],
  ...
  created() {
    this.selectbook = this.databook
    this.selectmovie = this.datamovie
    this.selectmusic = this.datamusic
    this.mylistbook = JSON.parse(localStorage.getItem('MyBookFav'));
    this.mylistmusic = JSON.parse(localStorage.getItem('MySongFav'));
    this.mylistmov = JSON.parse(localStorage.getItem('MyMovieFav'));
  },
  ...
  methods: {
    selecttype(type, fav) {
      if (fav === "book") {
        this.topicbook = type;
        this.selectbook = this.databook.filter((val) => {
          if (val.type === type) {
            return val;
          }
        })
        ...
        this.topicsong = type;
        this.selectmusic = this.datamusic.filter((val) => {
          if (val.type === type) {
            return val;
          }
        })
        ...
        this.topicmov = type;
        this.selectmovie = this.datamovie.filter((val) => {
          if (val.type === type) {
            return val;
          }
        })
    ...
    addfavlist(item, fav) {
      ...
      if (fav === "book") {
        let findmore = this.listbook.includes(item)

        if (!findmore) {
          return this.listbook.push(item);
          console.log(this.listbook)
        }
      } else if (fav === "music") {
        let findmore = this.listsongs.includes(item)

        if (!findmore) {
          return this.listsongs.push(item);
          console.log(this.listsongs)
        }
      } else if (fav === "movie") {
        let findmore = this.listmov.includes(item)

        if (!findmore) {
          return this.listmov.push(item);
        }
      }
    ...
    gotoprofile(type) {
      ...
      if (type === 'book') {
        const mybook = JSON.stringify(this.listbook)
        localStorage.setItem('MyBookFav', mybook)
      } else if (type === 'music') {
        const mysong = JSON.stringify(this.listsongs)
        localStorage.setItem('MySongFav', mysong)
      } else if (type === 'movie') {
        const mymovie = JSON.stringify(this.listmov)
        localStorage.setItem('MyMovieFav', mymovie)
      }
    ...
    allmylist() {
      ...
      console.log(this.mylistbook)
      count = 0;

      this.mylistbook.forEach((val) => {
        count++
        const myitem = {
          "id": count,
        "name": val.bookname,
        ...
        return this.alllist.push(myitem)
      })

      this.mylistmusic.forEach((val) => {
        count++
        const myitem = {
          "id": count,
        "name": val.songname,
  ...
  watch: {
    ...
    searchsongs: function (newValue, oldValue) {
      console.log(newValue)
      this.selectmusic = this.datamusic.filter((val) => {

        if (val.songname.toLowerCase().includes(newValue.toLowerCase())) {
          return val
        }
      })
    },

    searchmov: function (newValue, oldValue) {
      console.log(newValue)
      this.selectmovie = this.datamovie.filter((val) => {

        if (val.moviename.toLowerCase().includes(newValue.toLowerCase())) {
          return val
        }
      })
    }
