        ...
        <div class="chatting">
            ...
            <div v-for="msg in mss" :key="msg.id">
              <div class="chat-outcoming" v-if="msg.email === user.email">
                ...
                <img
                  class="user"
                  style="width: 4%"
                  src="http://localhost:3000/uploads/person.png"
                  alt=""
                />
              </div>
          ...
          <div class="sendmsg">
            <input
              id="textMsg"
              v-model="message"
              style="margin-top: 6%; margin-bottom: 2%"
              class="form-control"
              placeholder="ข้อความ"
              type="text"
            />
            <button id="btnSend" @click="submit()" class="btn-primary">
              ส่ง
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    submit() {
      ...
      axios
        .post("http://localhost:3000/bookchat", data)
        .then((res) => {
